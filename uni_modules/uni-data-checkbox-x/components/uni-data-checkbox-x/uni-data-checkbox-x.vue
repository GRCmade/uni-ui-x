...
<script>
	import {
		ArrStrNum2Arr,
		StrNum2NumInt
	} from '@/uni_modules/uni-types-x/index.uts';
	type dataListItem = {
		disabled ? : boolean;
		selected ? : boolean;
		styleBackgroud ? : string;
		styleIcon ? : string;
	...
	type dataListItemDetial = {
		data: dataListItem[]
		value: number[]
	}

	type DataCheckBoxXMap = {
		text: string
		value: string
	}

	const defaultDataCheckBoxX = {
		mode: "default",
		map: {
			text: "text",
			value: "value"
	...
	export default {
		...
		props: {
			...
			map: {
				type: Object as PropType < DataCheckBoxXMap > ,
				default: defaultDataCheckBoxX["map"]
			}
		},
		watch: {
			localdata: {
			...
			// },
			value(newVal) {
				this.dataList = this.getDataList(ArrStrNum2Arr < number > (newVal))
		...
		methods: {
			loadData() {
				// TODO 不确定这东西从哪来的mixinDatacomGet
				...
				// })
				// console.log(this.mixinDatacomErrorMessage);
			},
			/**
			 * TODO 实现form时看这个
			 * 获取父元素实例
			...
			change(e: string[]) {
				...
				if (this.multiple) {
					this.range.forEach((item: dataListItem) => {

						if (values.includes(item[this.map.value].toString())) {
							detail.value.push(item[this.map.value] as number)
							detail.data.push(item)
						}
					})
				} else {
					const range: dataListItem | null = this.range.find((item: dataListItem): boolean => item[this.map
						.value] as string == values as string)
					if (range != null) {
						detail = {
				...
				const emitValue: any = this.multiple ? detail.value : detail.value[0]
				// TODO 兼容 vue2
			...
			getDataList(value: number[]): dataListItem[] {
				...
				for (var i = 0; i < dataList?.length as number; i++) {
					let item: dataListItem = dataList[i]
					item.disabled = item.disabled == true || false
					if (this.multiple) {
						if (value.length > 0) {
							for (var j = 0; j < value.length; j++) {
								if (value[j] == item[this.map.value]) {
									item.selected = true
								}
							}
							// let have = value.find((val:number) => val == item[this.map.value])
							// item.selected = have !== null
						} else {
							item.selected = false
						}
					} else {
						// console.log(value[0],item[this.map.value])
						// TODO 需要将所有传入的数值都变成number[]这里的value，需要改成value[0]
			 ...
			 * @param {Object} list
			 */
			setRange(list: dataListItem[]): dataListItem[] {
				let selectList = list.filter((item: dataListItem): boolean => item.selected == true)
				let min = StrNum2NumInt(this.min) ?? 0
				let max = StrNum2NumInt(this.max) ?? 99999
			...
			setStyleRightIcon(item: dataListItem): string {
				...
				let classles = ''
				if (this.mode === 'list') {
					styles['border-color'] = item.selected == true ? this.styles["selectedColor"] : '#DCDFE6'
				}
				for (let i in styles) {
					classles += `${i}:${styles[i]};`
				}
