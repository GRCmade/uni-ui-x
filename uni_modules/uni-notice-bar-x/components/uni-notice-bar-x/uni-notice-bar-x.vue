<template>
	<view v-if="show" class="uni-noticebar" :style="{ backgroundColor }" @click="onClick">
		...
		<view ref="textBox" class="uni-noticebar__content-wrapper" :class="{
				...
				'uni-noticebar__content-wrapper--single': scrollable == false && (single== true || moreText != '')
			}" :style="{ height: scrollable ? fontSize * 1.5 + 'px' : 'auto' }">
			<view :id="elIdBox" class="uni-noticebar__content" :class="{
					'uni-noticebar__content--scrollable': scrollable == true,
					'uni-noticebar__content--single': scrollable == false && (single== true || moreText != '')
				}">
		...
		<view v-if="isShowGetMore" class="uni-noticebar__more uni-cursor-point" @click="clickMore">
			...
			<uni-icons-x v-else type="right" :color="moreColor" :size="fontSize * 1.1" />
		</view>
		<view class="uni-noticebar-close uni-cursor-point" v-if="isShowClose">
			<uni-icons-x type="closeempty" :color="color" :size="fontSize * 1.1" @click="close" />
		</view>
	</view>
</template>
		...
		methods: {
			initSize() {
				if (this.scrollable) {
					...
					let boxQuery = new Promise<string>((resolve, reject) => {
						uni.createSelectorQuery()
							.in(this)
							.select(`#${this.elIdBox}`)
							.boundingClientRect()
